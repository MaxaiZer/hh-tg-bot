...
func Test_RemoveSearchCmd_WhenInvalidInput_ShouldWaitForValid(t *testing.T) {
	...
	cmd := newRemoveSearchCommand(&mockApi{}, search.UserID, EventBus.New(), mockSearches)
	cmd.WithFinishCallback(func() { finished = true })
...
func Test_EditSearchCmd_WhenValidData_ShouldBeSuccessful(t *testing.T) {
	...
	cmd := newEditSearchCommand(&mockApi{}, search.UserID, mockBus, mockSearches)
	cmd.WithFinishCallback(func() { finished = true })
	...
	assert.Equal(mockSearches.Searches[0].SearchText, newKeywords)

	cmd.OnUserInput("1") //select changing of user wish
	cmd.OnUserInput(newWish)

	assert.False(finished)
	assert.Equal(mockSearches.Searches[0].UserWish, newWish)
}
...
func Test_EditSearchCmd_WhenInvalidInput_ShouldWaitForValid(t *testing.T) {
	...
	cmd := newEditSearchCommand(&mockApi{}, search.UserID, EventBus.New(), mockSearches)
	cmd.WithFinishCallback(func() { finished = true })
